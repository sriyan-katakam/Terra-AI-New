...
const DEFAULT_AVATAR = "data:image/svg+xml;base64,";
...
class TerraApp {
  constructor() {
    ...
    this.apiKey = localStorage.getItem('aether_api_key') ||
      ((OPENROUTER_API_KEY && OPENROUTER_API_KEY !== 'PASTE_YOUR_API_KEY_HERE') ? OPENROUTER_API_KEY : '');
  ...
  initElements() {
    ...
    this.profileApiKeyInput = document.getElementById('profileApiKeyInput');
  ...
  bindEvents() {
    ...
    if (this.btnRemovePhoto) {
      this.btnRemovePhoto.addEventListener('click', () => {
        ...
        if (this.settingsPhotoPreview) {
          this.settingsPhotoPreview.src = DEFAULT_AVATAR;
        }
      });
    }
  ...
  saveSettings() {
    ...
    const customApiKey = this.profileApiKeyInput ? this.profileApiKeyInput.value.trim() : '';
    ...
    if (customApiKey) {
      ...
      localStorage.setItem('aether_api_key', customApiKey);
    } else {
    ...
    if (this.customPhotoDataUrl) {
      ...
      this.user.photoURL = DEFAULT_AVATAR;
    }
  ...
  // --- SPEECH RECOGNITION (SPEECH-TO-TEXT) METHODS ---
  initSpeechRecognition() {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
      console.warn("Speech Recognition API is not supported in this browser.");
      if (this.micBtn) {
        this.micBtn.style.display = 'none'; // Hide mic button if browser is not supported
      }
      return;
    }
    ...
    this.recognition.onstart = () => {
      ...
      if (this.micBtn) {
        ...
        this.micBtn.setAttribute('title', 'Stop Listening');
      }
      this.showStatus('Listening... Speak now', 'info');
      this.systemStatus.style.color = 'var(--primary)';
    };
    ...
    this.recognition.onresult = (event) => {
      ...
      if (transcript && this.chatInput) {
        ...
        this.chatInput.value = currentText + separator + transcript;
        
        // Trigger manual input grow and count event
        this.chatInput.dispatchEvent(new Event('input'));
        this.showStatus('Voice text captured.', 'success');
      }
    };
    ...
    this.recognition.onerror = (event) => {
      ...
      if (this.micBtn) {
        ...
        this.micBtn.setAttribute('title', 'Voice Input (Speech-to-Text)');
      }
      
      if (event.error === 'not-allowed') {
        this.showStatus('Microphone access blocked. Enable permissions in browser.', 'error');
        alert('Microphone permission was denied. Please click the camera/mic icon in the browser address bar to allow microphone access.');
      } else if (event.error === 'network') {
        this.showStatus('Speech services require an active internet connection.', 'error');
      } else {
  ...
  updateAuthStateUI() {
    if (this.user) {
      // Migrate legacy Dicebear avatar URL to new custom silhouette avatar
      if (this.user.photoURL && (this.user.photoURL.includes('dicebear.com') || this.user.photoURL === '')) {
        this.user.photoURL = DEFAULT_AVATAR;
        localStorage.setItem('aether_user', JSON.stringify(this.user));
      }

      // Hydrate customPhotoDataUrl state if user has a custom picture
      if (this.user.photoURL && this.user.photoURL !== DEFAULT_AVATAR) {
      ...
      if (this.sidebarProfileCard) {
        ...
        if (this.profileAvatar) {
          this.profileAvatar.src = this.user.photoURL || DEFAULT_AVATAR;
        }
      ...
      if (this.settingsPhotoPreview) {
        this.settingsPhotoPreview.src = this.user.photoURL || DEFAULT_AVATAR;
      }
      ...
      if (this.profileApiKeyInput) {
        this.profileApiKeyInput.value = localStorage.getItem('aether_api_key') || '';
      }
    } else {
